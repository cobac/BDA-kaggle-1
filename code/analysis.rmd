...
```{r}
# afinn_dict <- get_sentiments("afinn") => Fails interactively
...
- this is a newly added feature and not 'stolen'
...
## Ratio of pauses ("-" in transcript) to sentences

- idea 'stolen' from Group 2

```{r}
pauses_df <- transcripts_df %>%
    group_by(vlogId) %>%
    full_join(count_sentences) %>%
    mutate(ratio_pauses = str_count(Text, pattern = "-") / total_sentences) %>%
